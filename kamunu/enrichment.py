...
def Qsearch(id_:str):
    ...
    if r:
        ...
        data_code = r['entities'][id_]['claims']['P1566'][0]['mainsnak']['datavalue']['value'] if 'P1566' in r['entities'][id_]['claims'] else None
...
def location_enrichment(record):
    ...
    if ror and 'country' in ror:
        country_name = ror['country']['country_name'] #Extraer el país desde ROR

        # Revisar si hay addresses en los datos de ROR
        if 'addresses' in ror:
            city_name = ror['addresses'][0]['city']
            geonames_city = [ror['addresses'][0]['geonames_city']['id'],
                            ror['addresses'][0]['geonames_city']['geonames_admin1']['id'] if ror['addresses'][0]['geonames_city']['geonames_admin1'] else None,
            ...
            coordinates = {'latitude': ror['addresses'][0]['lat'], 'longitude': ror['addresses'][0]['lng']}

            country = {'country_name': country_name, 'geonames_id': country_code}
    ...
    if wikidata and 'claims' in wikidata:
        ...
        if 'P17' in claims:
            ...
            country = {'country_name': country_data[0], 'country_code': country_data[1]}
        ...
        if 'P159' in claims and 'qualifiers' in claims['P159'][0]:
            latitude = claims['P159'][0]['qualifiers']['P625'][0]['datavalue']['value']['latitude'] if 'P625' in claims['P159'][0]['qualifiers'] else 0
            longitude = claims['P159'][0]['qualifiers']['P625'][0]['datavalue']['value']['longitude'] if 'P625' in claims['P159'][0]['qualifiers'] else 0
